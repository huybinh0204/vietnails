...
module.exports = {
    ...
    statistic_schedule: (req, res) => {
        let start_time = req.body.start_time ;
        let sql = `SELECT start_time,status,COUNT(*) AS sum_schedule , SUM(moneys) as sum_moneys,  created_schedule FROM schedule WHERE schedule.start_time LIKE '${start_time}%' GROUP BY schedule.status`;
        db.query(sql, (err, rown, fields) => {
            if (err) throw err
            var obj = [];
            for (var i = 0; i < rown.length; i++) {
