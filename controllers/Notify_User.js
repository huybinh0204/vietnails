...
module.exports = {
    ...
    get_notify_nv: (req, res) => {
        ...
        let sql = `SELECT * FROM schedule JOIN schedule_historical ON schedule.id = schedule_historical.id_schedule JOIN `+
        `notify_key ON schedule_historical.id_User = notify_key.id_User WHERE `+
        `schedule.start_time LIKE '${check_time_schedule}%' AND schedule_historical.is_status =1 GROUP BY schedule.id`;
